...
class MockCulturalEventRepository implements CulturalEventRepository {
  ...
  List<CulturalEvent> _buildEvents(LatLng center) {
    ...
    return [
      ...
      // ── 소멸 시스템 테스트 (종료 후 각 단계 확인용) ─────────────────────────
      // opacity ~0.92 — 방금 종료, 거의 불투명
      CulturalEvent(
        id: 'fade-001',
        title: '[소멸 테스트] 방금 종료 -5분',
        venue: '소멸 테스트 A',
        address: '현재 위치 북쪽 150m',
        description: '종료 후 5분 경과. opacity 약 92%. 색상 변화 없음.',
        startDate: now.subtract(const Duration(hours: 2)),
        endDateTime: now.subtract(const Duration(minutes: 5)),
        location: LatLng(center.latitude + 0.0014, center.longitude),
        category: EventCategory.show,
        isFree: true,
        source: EventSource.public,
      ),
      // opacity ~0.67 — 20분 경과, 뚜렷한 투명도
      CulturalEvent(
        id: 'fade-002',
        title: '[소멸 테스트] 종료 -20분',
        venue: '소멸 테스트 B',
        address: '현재 위치 동쪽 150m',
        description: '종료 후 20분 경과. opacity 약 67%. 아직 원래 색상.',
        startDate: now.subtract(const Duration(hours: 2)),
        endDateTime: now.subtract(const Duration(minutes: 20)),
        location: LatLng(center.latitude, center.longitude + 0.0018),
        category: EventCategory.theater,
        isFree: false,
        source: EventSource.public,
      ),
      // opacity ~0.42 — 35분 경과, 그레이톤 전환
      CulturalEvent(
        id: 'fade-003',
        title: '[소멸 테스트] 종료 -35분 (그레이)',
        venue: '소멸 테스트 C',
        address: '현재 위치 남쪽 150m',
        description: '종료 후 35분 경과. opacity 약 42%. 그레이톤 전환.',
        startDate: now.subtract(const Duration(hours: 2)),
        endDateTime: now.subtract(const Duration(minutes: 35)),
        location: LatLng(center.latitude - 0.0014, center.longitude),
        category: EventCategory.concert,
        isFree: false,
        source: EventSource.public,
      ),
      // opacity ~0.17 — 50분 경과, 거의 소멸
      CulturalEvent(
        id: 'fade-004',
        title: '[소멸 테스트] 종료 -50분 (거의 소멸)',
        venue: '소멸 테스트 D',
        address: '현재 위치 서쪽 150m',
        description: '종료 후 50분 경과. opacity 약 17%. 그레이톤 + 반투명.',
        startDate: now.subtract(const Duration(hours: 2)),
        endDateTime: now.subtract(const Duration(minutes: 50)),
        location: LatLng(center.latitude, center.longitude - 0.0018),
        category: EventCategory.exhibition,
        isFree: true,
        source: EventSource.public,
      ),
      // 파트너 소멸 테스트 — 펄스 마커 fade 확인
      CulturalEvent(
        id: 'fade-005',
        title: '[소멸 테스트] 파트너 -25분',
        venue: '소멸 테스트 파트너',
        address: '현재 위치 북동쪽 200m',
        description: '파트너 이벤트 종료 25분 경과. 펄스 마커 fade 확인.',
        startDate: now.subtract(const Duration(hours: 1)),
        endDateTime: now.subtract(const Duration(minutes: 25)),
        location: LatLng(center.latitude + 0.001, center.longitude + 0.001),
        category: EventCategory.partner,
        isFree: false,
        source: EventSource.partner,
      ),
